using AgileLabs.Dapper.FluentMap.SqlBuilders;
using Dapper;
using System;
...
{
    public static class AgileMapper
    {
        ...
        public static PagedList<TEntity> QueryPage<TEntity>(this IDbConnection connection,
            Expression<Func<TEntity, bool>> predicate,
            Expression<Func<TEntity, object>> orderByExpression = null,
            bool orderByAsc = true,
            int? pageNo = null, int? pageSize = null)
        {
            ...
            DynamicParameters paginationParameters;
            var paginationSql = BuildSelectSql<TEntity>(predicate, out paginationParameters);
            if (pageNo == null || pageSize == null)
            {
                paginationSql = $"{paginationSql} {orderBySql}";
            }
            else
            {
                paginationSql = BuildPaginationSql(connection, tableName, paginationSql, orderBySql, orderByAsc, pageNo.Value, pageSize.Value);
            }

            DynamicParameters parameters;
            var countSql = BuildCountSql(predicate, out parameters);

            using (var multi = connection.QueryMultiple($"{countSql};{paginationSql}", paginationParameters))
            {
                var totalCount = multi.Read<int>().Single();
                var entityList = multi.Read<TEntity>().ToList();
                return new PagedList<TEntity>(entityList, 1, totalCount, totalCount);
            }
        }

        //public static PagedList<TEntity> QueryPage<TEntity>(this IDbConnection connection,
        //    WhereDefinition<TEntity> filter, SortDefinition<TEntity> sort,
        //    int? pageNo = null, int? pageSize = null)
        //{
        //    var tableName = Resolvers.Table(typeof(TEntity));
        //    var orderBySql = orderSql;

        //    DynamicParameters paginationParameters;
        //    var paginationSql = BuildSelectSql<TEntity>(null, out paginationParameters);
        //    if (pageNo == null || pageSize == null)
        //    {
        //        paginationSql = $"{paginationSql} {orderBySql}";
        //    }
        //    else
        //    {
        //        paginationSql = BuildPaginationSql(connection, tableName, paginationSql, orderBySql, pageNo.Value, pageSize.Value);
        //    }

        //    DynamicParameters parameters;
        //    var countSql = BuildCountSql(null, out parameters);
        ...
        public static object InsertList<TEntity>(this IDbConnection connection, IEnumerable<TEntity> entity, IDbTransaction transaction = null) where TEntity : class
        {
            //var sql = BuildInsertQuery(connection, typeof(TEntity));
            //return connection.ExecuteScalar(sql, entity, transaction);
            var sql = BuildInsertQuery(connection, typeof(TEntity));
            return connection.Execute(sql, entity.ToArray(), transaction);
        }
        ...
        internal static void AddSqlBuilder(Type connectionType, ISqlBuilder builder)
        {
            _sqlBuilders[connectionType.Name.ToLower()] = builder;
        }

        internal static ISqlBuilder GetBuilder(IDbConnection connection)
        {
